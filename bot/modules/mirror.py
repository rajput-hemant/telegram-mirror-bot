        ...
        if (
            ...
            if file is None:
                ...
                if multi > 1:
                    ...
                    multi -= 1
                    sleep(3)
                    Thread(target=_mirror, args=(bot, nextmsg, isZip,
                           extract, isQbit, isLeech, pswd, multi)).start()
                return
            else:
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            ...
            Thread(target=add_gd_download, args=(
                link, listener, is_gdtot)).start()
    elif is_mega_link(link):
        if MEGA_KEY is not None:
            mega_dl = MegaDownloader(listener)
            Thread(target=mega_dl.add_download, args=(
        ...
        Thread(target=qb_dl.add_qb_torrent, args=(
            link, f'{DOWNLOAD_DIR}{listener.uid}', qbitsel)).start()
    else:
...
def qb_unzip_leech(update, context):
    _mirror(context.bot, update.message,
            extract=True, isQbit=True, isLeech=True)
